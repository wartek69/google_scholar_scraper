...
def get_page(pagerequest, session):
    """Return the data for a page on scholar.google.com"""
    # Note that we include a sleep to avoid overloading the scholar server
    #time.sleep(5+random.uniform(0, 5))
    request_url = 'http://api.scraperapi.com?api_key=hereyourapikey&url='+pagerequest+'&keep_headers=true'
    request_url_scrape_stack = 'https://api.scrapestack.com/scrape?access_key=hereyourapikey&url='+pagerequest+'&keep_headers=1'
    resp = requests.get(request_url, headers=_HEADERS, cookies=_COOKIES)
    i = 0
    while(resp.status_code != 200):
        if i > 3:
            if i > 10:
                ...
                return ""
            print('too many failed requests, trying scrapestack')
            resp = requests.get(request_url_scrape_stack, headers=_HEADERS, cookies=_COOKIES)

        else:
            print('retrying the request for {}...'.format(pagerequest))
            id = hashlib.md5(str(random.random()).encode('utf-8')).hexdigest()[:16]
            cookies = {'GSP': 'ID={0}:CF=4'.format(id)}
            resp = requests.get(request_url, headers=_HEADERS, cookies=cookies)

        i = i+1
        time.sleep(1)
    return resp.text

def _get_soup(pagerequest, session):
    """Return the BeautifulSoup for a page on scholar.google.com"""
    html = get_page(pagerequest, session)
    html = html.replace(u'\xa0', u' ')
    return BeautifulSoup(html, 'html.parser')


def _search_scholar_soup(soup, session):
    ...
    while True:
        ...
        if soup.find(class_='gs_ico gs_ico_nav_next'):
            ...
            soup = _get_soup(_HOST+url, session)
        else:
            break


def _search_citation_soup(soup, session):
...
class Publication(object):
    ...
    def fill(self, session):
        ...
        if self.source == 'citations':
            ...
            bibtex = get_page(self.url_scholarbib, session)
            #query_url = 'http://api.scraperapi.com?api_key=a6805e53ff13215056f0cfd864eeb760&url='+self.url_scholarbib
            #response = requests.get(query_url)
            #bibtex = response.text

            #if not response.status_code == 200:
                #print('something went wrong with the proxy')
                #print(bibtex)
                #return self
    ...
    def get_citedby(self, session):
        ...
        if hasattr(self, 'id_scholarcitedby'):
            ...
            soup = _get_soup(_HOST+url, session)
            return _search_scholar_soup(soup, session)
        else:
            return []
...
def search_pubs_custom_url(url, session):
    """Search by custom URL and return a generator of Publication objects
    URL should be of the form '/scholar?q=...'"""
    soup = _get_soup(_HOST+url, session)
    return _search_scholar_soup(soup, session)

def _search_scholar_soup_no_gen(soup):
    """Generator that returns Publication objects from the search page"""
    publications = []
    for row in soup.find_all('div', 'gs_or'):
        publications.append(Publication(row, 'scholar'))
    return publications

def search_pubs_custom_url_no_gen(url, session):
    """Search by custom URL and return a list of pub objects"""
    soup = _get_soup(_HOST+url, session)
    return _search_scholar_soup_no_gen(soup)